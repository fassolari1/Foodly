...
def trova_ingrediente_in_dispensa(nome_ricetta, dispensa):
    """
    Trova un ingrediente nella dispensa confrontando il nome della ricetta con quelli disponibili.
    Implementa logica flessibile: l'ingrediente della dispensa (singolare) deve essere contenuto
    nel nome dell'ingrediente della ricetta (che può essere plurale o con aggettivi).
    
    Args:
        nome_ricetta (str): Nome dell'ingrediente nella ricetta (es. "carrots", "sliced carrots")
        dispensa (dict): Dizionario della dispensa con nomi ingredienti come chiavi
    
    Returns:
        tuple: (nome_ingrediente_dispensa, quantita_disponibile) o (None, 0) se non trovato
    """
    nome_ricetta_lower = nome_ricetta.lower().strip()
    
    # Cerca direttamente prima (match esatto)
    if nome_ricetta in dispensa and dispensa[nome_ricetta] > 0:
        return nome_ricetta, dispensa[nome_ricetta]
    
    # Cerca con match flessibile: ingrediente dispensa contenuto in nome ricetta
    for nome_dispensa, quantita in dispensa.items():
        if quantita > 0:
            nome_dispensa_lower = nome_dispensa.lower().strip()
            # Verifica se il nome della dispensa è contenuto nel nome della ricetta
            if nome_dispensa_lower in nome_ricetta_lower:
                return nome_dispensa, quantita
...
def ricetta_realizzabile(ricetta, disp):
    ...
    for ingrediente_ricetta in ricetta["ingredients"]:
        ...
        if nome_dispensa and grammi_disponibili > 0: 
            # Converte la quantità richiesta in grammi (usando il nome della dispensa per la conversione)
...
def aggiorna_ingredienti(disp, ricetta, nome_ricetta_selezionata=None):
    ...
    for ingrediente_ricetta in ricetta["ingredients"]:
        ...
        if nome_dispensa and grammi_disponibili > 0:
            ...
            grammi_usati = converti_a_grammi(nome_dispensa, quantita_richiesta, unita)
            
            # Sottrae i grammi usati dalla dispensa
            disp[nome_dispensa] = max(0, disp[nome_dispensa] - grammi_usati)
